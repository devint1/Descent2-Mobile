		...
		if (goal_seg == -3) {
			...
			if ((aip->path_length > 0) && (Point_segs[aip->hide_index + aip->path_length - 1].segnum != goal_seg)) {
				...
				if (dist_to_player > MIN_ESCORT_DISTANCE)
					...
					create_n_segment_path(objp, 8 + drand() * 8, -1);
					aip->path_length = polish_path(objp, &Point_segs[aip->hide_index], aip->path_length);
				}
			}
		}
	...
	if (Escort_special_goal == ESCORT_GOAL_SCRAM) {
		if (player_visibility)
			if (Escort_last_path_created + F1_0*3 < GameTime) {
				...
				create_n_segment_path(objp, 10 + drand() * 16, ConsoleObject->segnum);
	...
	switch (ailp->mode) {
		...
		case AIM_SNIPE_FIRE:
			if (ailp->next_action_time < 0) {
				...
				create_n_segment_path(objp, 10 + drand()/2048, ConsoleObject->segnum);
				aip->path_length = polish_path(objp, &Point_segs[aip->hide_index], aip->path_length);
				if (drand() < 8192)
					ailp->mode = AIM_SNIPE_RETREAT_BACKWARDS;
				else
	...
	if ((Players[player_num].secondary_weapon_flags & HAS_FLAG(weapon_num)) && Players[player_num].secondary_ammo[weapon_num])
		if (drand() < THIEF_PROBABILITY) {
			if (weapon_num == PROXIMITY_INDEX)
				if (drand() > 8192)		//	Come in groups of 4, only add 1/4 of time.
					return 0;
...
int attempt_to_steal_item_2(object *objp, int player_num)
{
	...
	if (rval) {
		...
		if (drand() > 20000)	//	Occasionally, boost the value again
			Stolen_item_index = (Stolen_item_index+1) % MAX_STOLEN_ITEMS;
	}
...
int attempt_to_steal_item(object *objp, int player_num)
{
	...
	for (i=0; i<3; i++) {
		if (!rval || (drand() < 11000)) {	//	about 1/3 of time, steal another item
			rval += attempt_to_steal_item_2(objp, player_num);
		} else
			break;
	}
...
//	-------------------------------------------------------------------------------
//	Show the Buddy menu!
int show_escort_menu(char* goal_str, char* tstr)
{
	newmenu_item items[12];
	char goal[22];
	sprintf(goal, "Next goal: %s", goal_str);
	items[0].type = NM_TYPE_MENU;
	items[0].text = goal;
	items[1].type = NM_TYPE_MENU;
	items[1].text = "Find Energy Powerup";
	items[2].type = NM_TYPE_MENU;
	items[2].text = "Find Energy Center";
	items[3].type = NM_TYPE_MENU;
	items[3].text = "Find Shield Powerup";
	items[4].type = NM_TYPE_MENU;
	items[4].text = "Find Any Powerup";
	items[5].type = NM_TYPE_MENU;
	items[5].text = "Find a Robot";
	items[6].type = NM_TYPE_MENU;
	items[6].text = "Find a Hostage";
	items[7].type = NM_TYPE_MENU;
	items[7].text = "Stay Away From Me";
	items[8].type = NM_TYPE_MENU;
	items[8].text = "Find My Powerups";
	items[9].type = NM_TYPE_MENU;
	items[9].text = "Find the exit";
	items[10].type = NM_TYPE_MENU;
	items[10].text = tstr;
	items[11].type = NM_TYPE_MENU;
	items[11].text = "Rename Guide-Bot";
	return newmenu_do1(NULL, "Guide-Bot Menu", 12, items, NULL, -1) + 1;
}
...
void do_escort_menu(void)
{
	...
	if (!Buddy_messages_suppressed)
		sprintf(tstr, "Suppress Messages");
	else
		sprintf(tstr, "Enable Messages");
	
	In_screen = true;
	choice = show_escort_menu(goal_str, tstr);		//TXT_PAUSE);
	...
	if (choice == 11) {
		...
		Looking_for_marker = -1;
		Last_buddy_key = -1;
		set_escort_special_goal(choice);
		Last_buddy_key = -1;
		paused = 0;
	}
	In_screen = false;
