...
//------------ Read pitch_time -----------
	if ( !slide_on )	{
		...
		// From Cyberman...
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			kp += mouse_button_down_time(MB_PITCH_FORWARD, &x, &y)/(PH_SCALE*2);
			kp -= mouse_button_down_time(MB_PITCH_BACKWARD, &x, &y)/(PH_SCALE*2);
		}
		...
		if ( (use_mouse)&&(kc_mouse[13].value < 255) )	{
			if ( !kc_mouse[14].value )		// If not inverted...
				Controls.pitch_time -= (mouse_axis[kc_mouse[13].value]*Config_joystick_sensitivity)/3;
			else
				Controls.pitch_time += (mouse_axis[kc_mouse[13].value]*Config_joystick_sensitivity)/3;
		}
	} else {
...
//----------- Read vertical_thrust_time -----------------
	...
	if ( slide_on )	{
		...
		// From Cyberman...
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			Controls.vertical_thrust_time -= mouse_button_down_time(MB_PITCH_FORWARD, &x, &y);
			Controls.vertical_thrust_time += mouse_button_down_time(MB_PITCH_BACKWARD, &x, &y);
		}
	...
	// From mouse buttons
	if ( (use_mouse)&&(kc_mouse[8].value < 255 )) Controls.vertical_thrust_time += mouse_button_down_time( kc_mouse[8].value, &x, &y );
	if ( (use_mouse)&&(kc_mouse[9].value < 255 )) Controls.vertical_thrust_time -= mouse_button_down_time( kc_mouse[9].value, &x, &y  );
	...
	// From Cyberman...
	if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
		Controls.vertical_thrust_time += mouse_button_down_time(MB_Z_UP, &x, &y )/2;
		Controls.vertical_thrust_time -= mouse_button_down_time(MB_Z_DOWN, &x, &y )/2;
	}
...
//---------- Read heading_time -----------
	...
	if (!slide_on && !bank_on)	{
		...
		// From Cyberman...
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			kh -= mouse_button_down_time(MB_HEAD_LEFT, &x, &y )/PH_SCALE;
			kh += mouse_button_down_time(MB_HEAD_RIGHT, &x, &y )/PH_SCALE;
		}
		...
		if ( (use_mouse)&&(kc_mouse[15].value < 255 ))	{
			if ( !kc_mouse[16].value )		// If not inverted...
				Controls.heading_time += (mouse_axis[kc_mouse[15].value]*Config_joystick_sensitivity)/3;
			else
				Controls.heading_time -= (mouse_axis[kc_mouse[15].value]*Config_joystick_sensitivity)/3;
		}
	} else {
...
//----------- Read sideways_thrust_time -----------------
	...
	if ( slide_on )	{
		...
		// From cyberman
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			Controls.sideways_thrust_time -= mouse_button_down_time(MB_HEAD_LEFT, &x, &y );
			Controls.sideways_thrust_time += mouse_button_down_time(MB_HEAD_RIGHT, &x, &y );
		}
	...
	// From mouse buttons
	if ( (use_mouse)&&(kc_mouse[6].value < 255 )) Controls.sideways_thrust_time -= mouse_button_down_time( kc_mouse[6].value, &x, &y  );
	if ( (use_mouse)&&(kc_mouse[7].value < 255 )) Controls.sideways_thrust_time += mouse_button_down_time( kc_mouse[7].value, &x, &y  );
...
//----------- Read bank_time -----------------
	...
	if ( bank_on )	{
		...
		// From Cyberman...
		if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
			Controls.bank_time -= mouse_button_down_time(MB_HEAD_LEFT, &x, &y );
			Controls.bank_time += mouse_button_down_time(MB_HEAD_RIGHT, &x, &y );
		}
		...
		// From mouse...
		// HACK ALERT: axis index is not coming from kconfig. It is hard coded, since we don't expect
		// the iOS controls to change.
		if ( (use_mouse)&&(kc_mouse[15].value < 255 ))	{
			if ( !kc_mouse[16].value )		// If not inverted...
				Controls.bank_time += (mouse_axis[2]*Config_joystick_sensitivity)/8;
			else
				Controls.bank_time -= (mouse_axis[2]*Config_joystick_sensitivity)/8;
		}
	}
	...
	// From mouse buttons
	if ( (use_mouse)&&(kc_mouse[11].value < 255 )) Controls.bank_time += mouse_button_down_time( kc_mouse[11].value, &x, &y  );
	if ( (use_mouse)&&(kc_mouse[12].value < 255 )) Controls.bank_time -= mouse_button_down_time( kc_mouse[12].value, &x, &y  );
	...
	// From Cyberman
	if ((use_mouse)&&(Config_control_type==CONTROL_CYBERMAN))	{
		Controls.bank_time += mouse_button_down_time(MB_BANK_LEFT, &x, &y );
		Controls.bank_time -= mouse_button_down_time(MB_BANK_RIGHT, &x, &y );
	}
...
//----------- Read forward_thrust_time -------------
	...
	// From mouse buttons
	if ( (use_mouse)&&(kc_mouse[2].value < 255 )) Controls.forward_thrust_time += mouse_button_down_time( kc_mouse[2].value, &x, &y  );
	if ( (use_mouse)&&(kc_mouse[3].value < 255 )) Controls.forward_thrust_time -= mouse_button_down_time( kc_mouse[3].value, &x, &y  );
...
//----------- Read fire_primary_down_count
	...
	if ((use_mouse)&&(kc_mouse[0].value < 255 )) Controls.fire_primary_down_count += mouse_button_down_count(kc_mouse[0].value, &x, &y);
...
//----------- Read fire_secondary_down_count
	...
	if ((use_mouse)&&(kc_mouse[1].value < 255 )) Controls.fire_secondary_down_count += mouse_button_down_count(kc_mouse[1].value, &x, &y);
...
//----------- Read fire_flare_down_count
	...
	if ((use_mouse)&&(kc_mouse[4].value < 255 )) Controls.fire_flare_down_count += mouse_button_down_count(kc_mouse[4].value, &x, &y);

//----------- Read drop_bomb_down_count
	if (kc_keyboard[34].value < 255 ) Controls.drop_bomb_down_count += key_down_count(kc_keyboard[34].value);
	if (kc_keyboard[35].value < 255 ) Controls.drop_bomb_down_count += key_down_count(kc_keyboard[35].value);
	if ((use_joystick)&&(kc_joystick[26].value < 255 )) Controls.drop_bomb_down_count += joy_get_button_down_cnt(kc_joystick[26].value);
	if ((use_mouse)&&(kc_mouse[26].value < 255 )) Controls.drop_bomb_down_count += mouse_button_down_count(kc_mouse[26].value, &x, &y);

//----------- Read rear_view_down_count
	if (kc_keyboard[36].value < 255 ) Controls.rear_view_down_count += key_down_count(kc_keyboard[36].value);
	if (kc_keyboard[37].value < 255 ) Controls.rear_view_down_count += key_down_count(kc_keyboard[37].value);
	if ((use_joystick)&&(kc_joystick[25].value < 255 )) Controls.rear_view_down_count += joy_get_button_down_cnt(kc_joystick[25].value);
	if ((use_mouse)&&(kc_mouse[25].value < 255 )) Controls.rear_view_down_count += mouse_button_down_count(kc_mouse[25].value, &x, &y);
...
#ifndef MACINTOSH	// no mac support for vr headset
