...
static char rcsid[] = "$Id: newdemo.c 2.66 1996/09/20 14:51:50 jeremy Exp $";
...
{  
	...
	#else
		Newdemo_size = UINT_MAX;//GetDiskFree();
	#endif
...
void newdemo_start_playback(char * filename)
{
   ...
   First_time_playback=1;
	...
	if (filename==NULL) {
		...
		RandFileNum = drand() % NumFiles;
		NumFiles = 0;
		if ((dir = opendir( Document_path )) != NULL)	{
			while ((ent = readdir( dir )) != NULL) {
				if (NumFiles==RandFileNum && !strcasestr(ent->d_name, "dem")) {
					strcpy(filename3, ent->d_name);
					break;
				}
				NumFiles++;
			}
