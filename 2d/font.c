...
void get_char_width(unsigned int c,unsigned int c2,int *width,int *spacing)
{
	unsigned int letter;
...
int get_centered_x_scaled(char *s)
{
	int w,w2,s2;
	...
	return ((grd_curcanv->cv_bitmap.bm_w - w * f2fl(Scale_factor)) / 2);
}
...
int gr_ustring_unscaled(int x, int y, char *s )
{
	if (FFLAGS & FT_COLOR) {
...
void gr_get_string_size_unscaled(unsigned char *s, int *string_width, int *string_height, int *average_width )
{
...
void gr_get_string_size(char *s, int *string_width, int *string_height, int *average_width ) {
	int width, height, avg_width;

	gr_get_string_size_unscaled(s, &width, &height, &avg_width);
	width *= f2fl(Scale_factor);
	height *= f2fl(Scale_factor);
	avg_width *= f2fl(Scale_factor);
	*string_width = width;
	*string_height = height;
	*average_width = avg_width;
}

int gr_string_unscaled(int x, int y, char *s )
{
	int w, h, aw;
	int clipped=0;
	...
	if ( x == 0x8000 )	{
		if ( y<0 ) clipped |= 1;
		gr_get_string_size_unscaled(s, &w, &h, &aw );
	...
	if ( !clipped )
		return gr_ustring_unscaled(x, y, s );
...
int gr_scale_string(int x, int y, fix scale_x, fix scale_y, unsigned char * s) {
	int retval;
	int w, h, aw;
	...
	if (grd_curcanv->cv_bitmap.bm_type == BM_OGLES) {
		return gr_scale_string_ogles(x, y, scale_x, scale_y, s);
	}
#endif
	...
	retval = gr_string_unscaled(x == 0x8000 ? x : 0, 0, s);
...
int gr_string(int x, int y, char *s ) {
	return gr_scale_string(x, y, Scale_factor, Scale_factor, (unsigned char*)s);
}
...
// Needed to work with 64-bit
void read_font(grs_font *font, CFILE *fontfile) {
	int ft_data, ft_chars, ft_widths, ft_kerndata;
	
	cfread(&font->ft_w, sizeof(short), 1, fontfile);
	cfread(&font->ft_h, sizeof(short), 1, fontfile);
	cfread(&font->ft_flags, sizeof(short), 1, fontfile);
	cfread(&font->ft_baseline, sizeof(short), 1, fontfile);
	cfread(&font->ft_minchar, sizeof(ubyte), 1, fontfile);
	cfread(&font->ft_maxchar, sizeof(ubyte), 1, fontfile);
	cfread(&font->ft_bytewidth, sizeof(short), 1, fontfile);
	cfread(&ft_data, sizeof(int), 1, fontfile);
	cfread(&ft_chars, sizeof(int), 1, fontfile);
	cfread(&ft_widths, sizeof(int), 1, fontfile);
	cfread(&ft_kerndata, sizeof(int), 1, fontfile);
	
	font->ft_data = (ubyte *)ft_data;
	font->ft_chars = (ubyte **)ft_chars;
	font->ft_widths = (short *)ft_widths;
	font->ft_kerndata = (ubyte *)ft_kerndata;
}
...
grs_font * gr_init_font( char * fontname )
{
	...
	font = (grs_font *) malloc(datasize + FONT_HEADER_OFFSET);

	open_font[fontnum].ptr = font;

	read_font(font, fontfile);
	
	cfread(&((ubyte*)font)[FONT_HEADER_SIZE + FONT_HEADER_OFFSET],1,datasize-FONT_HEADER_SIZE,fontfile);
...
// gotta translate those endian things
	...
	if (font->ft_flags & FT_PROPORTIONAL) {
		...
		font->ft_widths = (short *) (((long) font->ft_widths) + FONT_HEADER_OFFSET + ((ubyte *) font));
		...
		font->ft_data = ((long) font->ft_data) + FONT_HEADER_OFFSET + ((ubyte *) font);
	...
	if (font->ft_flags & FT_KERNED)
		font->ft_kerndata = ((long) font->ft_kerndata) + FONT_HEADER_OFFSET + ((ubyte *) font);
...
void gr_remap_font( grs_font *font, char * fontname )
{
	...
	data_ofs = font->ft_data - ((ubyte *) font) - FONT_HEADER_OFFSET;
...
#ifdef OGLES
	font->ft_ogles_texes = realloc(font->ft_ogles_texes, sizeof(GLuint) * (font->ft_maxchar - font->ft_minchar));
	for (i = 0; i < font->ft_maxchar - font->ft_minchar; ++i) {
		glDeleteTextures(1, &font->ft_ogles_texes[i]);
		font->ft_ogles_texes[i] = 0;
	}
#endif
