...
static int Initialized = 0;
static size_t SmallestAddress = SIZE_MAX;
static size_t LargestAddress = 0x0;
...
static size_t MallocSizes[1024];

void * mem_malloc( size_t size, char * var, char * filename, int line, int fill_zero )
{
	unsigned long base;
	void *ptr;
...
void mem_free( void * buffer )
{
	...
	if (ErrorCount)	{
		fprintf( stderr, "\nMEM_OVERWRITE: Memory after the end of allocated block overwritten.\n" );
		fprintf( stderr, "\tBlock at %p, size %zu\n", buffer, MallocSizes[CurrentMalloc] );
		fprintf( stderr, "\t%d/%d check bytes were overwritten.\n", ErrorCount, CHECKSIZE );
	}
