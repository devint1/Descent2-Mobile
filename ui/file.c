...
int file_getdirlist( int MaxNum, char list[][13] )
{
	...
	if (fhandle = FindFirstFile("*.", FILE_ATTRIBUTE_DIRECTORY, &find) == INVALID_HANDLE_VALUE)
	{
		if (find.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)	{
			if (strcmp( "..", find.cFileName) && strcmp( ".", find.cFileName))
				strncpy(list[NumDirs++], find.cFileName, 13 );
		}

		while( !FindNextFile( fhandle, &find ) )
		{
			if (find.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)	{
				if (strcmp( "..", find.cFileName) && strcmp( ".", find.cFileName))
				{
					if (NumDirs==74)
					{
						DMessageBox( -2,-2, 1, "Only the first 74 directories will be displayed.", "Ok" );
						break;
					} else {
						strncpy(list[NumDirs++], find.cFileName, 13 );
					}
...
int file_getfilelist( int MaxNum, char list[][13], char * filespec )
{
	...
	if( fhandle = FindFirstFile( filespec, 0, &find ) == INVALID_HANDLE_VALUE )
	{
		//if ( !(find.attrib & _A_SUBDIR) )
			strncpy(list[NumFiles++], find.cFileName, 13 );
		...
		while( !FindNextFile(fhandle, &find ) )
		{
			...
			//{
				if (NumFiles==300)
				{
					DMessageBox( -2,-2, 1, "Only the first 300 files will be displayed.", "Ok" );
					break;
				} else {
					strncpy(list[NumFiles++], find.cFileName, 13 );
				}
...
int ui_get_filename( char * filename, char * Filespec, char * message  )
{
	...
	while( 1 )
	{
		...
		if ( HelpButton->pressed )
			DMessageBox( -1, -1, 1, "Sorry, no help is available!", "Ok" );
		...
		if (Button1->pressed || UserFile->pressed || (ListBox1->selected_item > -1 ) || (ListBox2->selected_item > -1 ))
		{
			...
			if (file_chdir( fulldir )==0)
			{
				...
				DMessageBox( -2, -2, 1, ErrorMessage, "Ok" );
				UserFile->first_time = 1;
