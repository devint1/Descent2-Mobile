...
static char rcsid[] = "$Id: weapon.c 2.86 1996/05/15 17:08:09 jed Exp $";
...
int pick_up_secondary(int weapon_index,int count)
{
	...
	cutpoint=255;
	if ((weapon_index<cutpoint && (weapon_index < Secondary_weapon)) || (Players[Player_num].secondary_ammo[Secondary_weapon] == 0))
	...
	for (i=0; i<MAX_SMEGA_DETONATES; i++) {
		...
		if (Smega_detonate_times[i] != 0) {
			...
			if (!Seismic_sound_playing) {
				...
				Next_seismic_sound_time = GameTime + drand()/2;
			}
			...
			if (delta_time < SMEGA_SHAKE_TIME) {
				...
				rx = fixmul(drand() - 16384, 3*F1_0/16 + (F1_0*(16-fc))/32);
				rz = fixmul(drand() - 16384, 3*F1_0/16 + (F1_0*(16-fc))/32);
...
int start_seismic_disturbance(void)
{
	...
	rval =  (2 * fixmul(drand(), Level_shake_frequency)) < FrameTime;
...
int spit_powerup(object *spitter, int id,int seed)
{
	...
	new_velocity.x += (drand() - 16384) * SPIT_SPEED * 2;
	new_velocity.y += (drand() - 16384) * SPIT_SPEED * 2;
	new_velocity.z += (drand() - 16384) * SPIT_SPEED * 2;
...
void do_seismic_stuff(void)
{
	...
	if (stv_save != 0) {
		...
		if ((GameTime > Next_seismic_sound_time) && Seismic_tremor_volume) {
			...
			Next_seismic_sound_time = GameTime + drand()/4 + 8192;
		}
	}
