...
{
	Briefing_text_x = x * f2fl(Scale_x);
	Briefing_text_y = y * f2fl(Scale_y);
...
void show_bitmap_frame(void)
{
	...
	if (Bitmap_name[0] != 0) {
		...
		switch (Animating_bitmap_type) {
			case 0:
				bitmap_canv = gr_create_sub_canvas(grd_curcanv, 220, 45, 64, 64);
				break;
			case 1:
				bitmap_canv = gr_create_sub_canvas(grd_curcanv, 220, 45, 94, 94);
				break;
...
int load_briefing_screen( int screen_num )
{
...
#ifdef OGLES
	briefing_bm.bm_ogles_tex_id = 0;
#endif
	if ((pcx_error=pcx_read_bitmap( CurBriefScreenName, &briefing_bm, BM_LINEAR, New_pal ))!=PCX_ERROR_NONE)     {
	...
	scale_bitmap(&briefing_bm, scale_pts, 0);
...
void DoBriefingColorStuff ()
{
	...
	Erase_color = gr_find_closest_color_current(5, 5, 5);
}
...
int load_new_briefing_screen( char *fname )
{
	...
	briefing_bm.bm_ogles_tex_id = 0;
#endif
	if ((pcx_error=pcx_read_bitmap( fname, &briefing_bm, BM_LINEAR, New_pal ))!=PCX_ERROR_NONE)     {
...
int DefineBriefingBox (char **buf)
{
	...
	Briefing_screens[n].level_num=get_message_num (buf);
	Briefing_screens[n].message_num=get_message_num (buf);
	Briefing_screens[n].text_ulx=get_message_num (buf);
	Briefing_screens[n].text_uly=get_message_num (buf);
	Briefing_screens[n].text_width=get_message_num (buf);
...
int show_briefing_message(int screen_num, char *message)
{
	...
	char spinRobotName[8],kludge;  // matt don't change this!
	char fname[15];
	char DumbAdjust=0;
	char chattering=0;
	...
	bsp->text_ulx *= f2fl(Scale_x);
	bsp->text_uly *= f2fl(Scale_y);
	...
	while (!done) {
		...
		if (ch == '$') {
			...
			else if (ch=='U')
			{
				...
				start_time = timer_get_fixed_seconds();
				while ( (keypress = local_key_inkey()) == -1 ) {		//	Wait for a key
#ifdef WINDOWS
					if (_RedrawScreen) {
						_RedrawScreen = FALSE;
						hum_channel  = digi_start_sound( digi_xlat_sound(SOUND_BRIEFING_HUM), F1_0/2, 0xFFFF/2, 1, -1, -1, -1 );
			...
			if (Briefing_text_x - bsp->text_ulx * f2fl(Scale_x) < tab_stop)
				Briefing_text_x = bsp->text_ulx * f2fl(Scale_x) + tab_stop;
			...
			if (prev_ch != '\\') {
				prev_ch = ch;
				if (DumbAdjust==0)
					Briefing_text_y += (8*(MenuHires+1)) * f2fl(Scale_y);
				else
					DumbAdjust--;
				Briefing_text_x = bsp->text_ulx * f2fl(Scale_x);
				if (Briefing_text_y / f2fl(Scale_y) > bsp->text_uly + bsp->text_height) {
		...
		if (Briefing_text_x / f2fl(Scale_x) > bsp->text_ulx + bsp->text_width) {
			Briefing_text_x = bsp->text_ulx * f2fl(Scale_x);
			Briefing_text_y += bsp->text_uly * f2fl(Scale_y);
		}
