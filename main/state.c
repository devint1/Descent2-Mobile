...
void state_callback(int nitems,newmenu_item * items, int * last_key, int citem)
{
	...
	if (MenuHires) {
		x = fl2f((grd_curcanv->cv_bitmap.bm_w - THUMBNAIL_W * 2 * f2fl(Scale_x)) / 2);
...
int state_get_save_file(char * fname, char * dsc, int multi )
{
	...
	char filename[NUM_SAVES+1][FILENAME_MAX];
...
int state_get_restore_file(char * fname, int multi)
{
	...
	newmenu_item m[NUM_SAVES+2];
	char filename[NUM_SAVES+1][PATH_MAX];
	char desc[NUM_SAVES+1][DESC_LENGTH + 16];
	char id[5];
	int valid;
...
int state_save_all(int between_levels, int secret_save, char *filename_override)
{
	...
	char	filename[FILENAME_MAX], desc[DESC_LENGTH+1];
	...
	if (secret_save == 1) {
		...
		sprintf(filename_override, "%s/%s", Document_path, SECRETB_FILENAME);
	} else if (secret_save == 2) {
		filename_override = filename;
		sprintf(filename_override, "%s/%s", Document_path, SECRETC_FILENAME);
	} else {
		if (filename_override) {
			sprintf( filename, "%s/%s", Document_path, filename_override);
