...
static char rcsid[] = "$Id: ai2.c 1.72 1996/09/17 14:52:15 matt Exp $";
...
// -- unused, 08/07/95 -- // -- MK, 06/09/95		if (drand() > 0x7400) {
...
void set_next_fire_time(object *objp, ai_local *ailp, robot_info *robptr, int gun_num)
{
	...
	if ((gun_num != 0) || (robptr->weapon_type2 == -1))
		if ((objp->ctype.ai_info.behavior != AIB_SNIPE) || (drand() > 16384))
...
void ai_fire_laser_at_player(object *obj, vms_vector *fire_point, int gun_num, vms_vector *believed_player_pos)
{
	...
	if (Players[Player_num].flags & PLAYER_FLAGS_CLOAKED) {
		...
		if (GameTime - cloak_time > CLOAK_TIME_MAX/4)
			if (drand() > fixdiv(GameTime - cloak_time, CLOAK_TIME_MAX)/2) {
	...
	while ((count < 4) && (dot < F1_0/4)) {
		bpp_diff.x = believed_player_pos->x + fixmul((drand()-16384) * (NDL-Difficulty_level-1) * 4, aim);
		bpp_diff.y = believed_player_pos->y + fixmul((drand()-16384) * (NDL-Difficulty_level-1) * 4, aim);
		bpp_diff.z = believed_player_pos->z + fixmul((drand()-16384) * (NDL-Difficulty_level-1) * 4, aim);
...
{
	vec->x = (drand() - 16384) | 1;	// make sure we don't create null vector
	vec->y = drand() - 16384;
	vec->z = drand() - 16384;
			...
			switch (objp->ctype.ai_info.behavior) {
				case AIB_STILL:
				{
					...
					if (r < 4096) {
						...
						create_n_segment_path(objp, drand()/8192 + 2, -1);
		...
		if (Players[Player_num].flags & PLAYER_FLAGS_CLOAKED) {
			...
			*player_visibility = player_is_visible_from_object(objp, pos, robptr->field_of_view[Difficulty_level], vec_to_player);
			...
			if ((ailp->next_misc_sound_time < GameTime) && ((ailp->next_fire < F1_0) || (ailp->next_fire2 < F1_0)) && (dist < F1_0*20)) {
				// mprintf((0, "ANGRY! "));
				ailp->next_misc_sound_time = GameTime + (drand() + F1_0) * (7 - Difficulty_level) / 1;
			...
			*player_visibility = player_is_visible_from_object(objp, pos, robptr->field_of_view[Difficulty_level], vec_to_player);
				...
				if (ailp->previous_visibility == 0) {
					if (ailp->time_player_seen + F1_0/2 < GameTime) {
						...
						// -- else
							digi_link_sound_to_pos( robptr->see_sound, objp->segnum, 0, pos, 0 , Robot_sound_volume);
						ailp->time_player_sound_attacked = GameTime;
						ailp->next_misc_sound_time = GameTime + F1_0 + drand()*4;
					}
			...
			if ((*player_visibility == 2) && (ailp->next_misc_sound_time < GameTime)) {
				// -- mprintf((0, "ATTACK! "));
				ailp->next_misc_sound_time = GameTime + (drand() + F1_0) * (7 - Difficulty_level) / 2;
				// -- if (Player_exploded)
				// -- 	digi_link_sound_to_pos( robptr->taunt_sound, objp->segnum, 0, pos, 0 , Robot_sound_volume);
				// -- else
					digi_link_sound_to_pos( robptr->attack_sound, objp->segnum, 0, pos, 0 , Robot_sound_volume);
...
void teleport_boss(object *objp)
{
	...
	rand_index = (drand() * Num_boss_teleport_segs) >> 15;
...
int boss_spew_robot(object *objp, vms_vector *pos)
{
	...
	objnum = create_gated_robot( segnum, Spew_bots[boss_index][(Max_spew_bots[boss_index] * drand()) >> 15], pos);
	...
	if (objnum != -1) {
		...
		if (force_val) {
			...
			newobjp->mtype.phys_info.rotthrust.x = ((drand() - 16384) * force_val)/16;
			newobjp->mtype.phys_info.rotthrust.y = ((drand() - 16384) * force_val)/16;
			newobjp->mtype.phys_info.rotthrust.z = ((drand() - 16384) * force_val)/16;
...
{
	if (segnum < 0)
		segnum = Boss_gate_segs[(drand() * Num_boss_gate_segs) >> 15];
...
int do_robot_dying_frame(object *objp, fix start_time, fix roll_duration, byte *dying_sound_playing, int death_sound, fix expl_scale, fix sound_scale)
{
	...
	if (start_time + roll_duration - sound_duration < GameTime) {
		...
		} else if (drand() < FrameTime*16)
			create_small_fireball_on_object(objp, (F1_0 + drand()) * (16 * expl_scale/F1_0)/8, 0);
	} else if (drand() < FrameTime*8)
		create_small_fireball_on_object(objp, (F1_0/2 + drand()) * (16 * expl_scale/F1_0)/8, 1);
...
void ai_do_actual_firing_stuff(object *obj, ai_static *aip, ai_local *ailp, robot_info *robptr, vms_vector *vec_to_player, fix dist_to_player, vms_vector *gun_point, int player_visibility, int object_animates, int gun_num)
{
	...
	//	---------------------------------------------------------------
		...
		if (drand()/2 < fixmul(FrameTime, (Difficulty_level << 12) + 0x4000)) {
