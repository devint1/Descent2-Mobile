							...
							case OBJ_ROBOT:
								...
								if ((objp != NULL) && (!Robot_info[obj0p->id].boss_flag) && (Weapon_info[objp->id].flash)) {
									...
									if (obj->ctype.ai_info.SKIP_AI_COUNT * FrameTime < F1_0) {
										...
										obj0p->mtype.phys_info.rotthrust.x = ((drand() - 16384) * force_val)/16;
										obj0p->mtype.phys_info.rotthrust.y = ((drand() - 16384) * force_val)/16;
										obj0p->mtype.phys_info.rotthrust.z = ((drand() - 16384) * force_val)/16;
...
object *object_create_debris(object *parent, int subobj_num)
{
	...
	obj->mtype.phys_info.velocity.x = D_RAND_MAX/2 - drand();
	obj->mtype.phys_info.velocity.y = D_RAND_MAX/2 - drand();
	obj->mtype.phys_info.velocity.z = D_RAND_MAX/2 - drand();
	vm_vec_normalize_quick(&obj->mtype.phys_info.velocity);
	vm_vec_scale(&obj->mtype.phys_info.velocity,i2f(10 + (30 * drand() / D_RAND_MAX)));
	...
	vm_vec_make(&obj->mtype.phys_info.rotvel, drand() + 0x1000, drand()*2 + 0x4000, drand()*3 + 0x2000);
	vm_vec_zero(&obj->mtype.phys_info.rotthrust);

	obj->lifeleft = 3*DEBRIS_LIFE/4 + fixmul(drand(), DEBRIS_LIFE);	//	Some randomness, so they don't all go away at the same time.
...
//	mprintf((0, "Finding a segment %i segments away from segment %i: ", max_depth, objp->segnum));
	...
	for (i=0; i<4; i++) {
		...
		ind1 = (drand() * MAX_SIDES_PER_SEGMENT) >> 15;
		temp = side_rand[ind1];
...
//			mprintf((0, "selected segment %i\n", seg_queue[tail]));
		...
		ind1 = (drand() * MAX_SIDES_PER_SEGMENT) >> 15;
		ind2 = (drand() * MAX_SIDES_PER_SEGMENT) >> 15;
		temp = side_rand[ind1];
...
int choose_drop_segment()
{
	...
	cur_drop_depth = BASE_NET_DROP_DEPTH + ((drand() * BASE_NET_DROP_DEPTH*2) >> 15);
	...
	while ((segnum == -1) && (cur_drop_depth > BASE_NET_DROP_DEPTH/2)) {
		...
		if (count == N_players) {
			...
			//return (drand() * Highest_segment_index) >> 15;
		}
...
//--old-- 		segnum = (drand() * Highest_segment_index) >> 15;
	...
	switch (type) {
		case OBJ_POWERUP:
			for (count=0; count<num; count++) {
				int	rand_scale;
				memcpy(&new_velocity, init_vel, sizeof(new_velocity));
				...
				new_velocity.x += fixmul(old_mag+F1_0*32, drand()*rand_scale - 16384*rand_scale);
				new_velocity.y += fixmul(old_mag+F1_0*32, drand()*rand_scale - 16384*rand_scale);
				new_velocity.z += fixmul(old_mag+F1_0*32, drand()*rand_scale - 16384*rand_scale);
				...
				memcpy(&new_pos, pos, sizeof(new_pos));
//				new_pos.x += (drand()-16384)*8;
//				new_pos.y += (drand()-16384)*8;
//				new_pos.z += (drand()-16384)*8;
...
//				mprintf(0, "                             pos = x=%d y=%d z=%d\n", obj->pos.x, obj->pos.y, obj->pos.z);
				...
				switch (obj->id) {
					...
					case POW_ENERGY:
						obj->lifeleft = (drand() + F1_0*3) * 64;		//	Lives for 3 to 3.5 binary minutes (a binary minute is 64 seconds)
						if (Game_mode & GM_MULTI)
							obj->lifeleft /= 2;
						break;
					default:
//						if (Game_mode & GM_MULTI)
//							obj->lifeleft = (drand() + F1_0*3) * 64;		//	Lives for 5 to 5.5 binary minutes (a binary minute is 64 seconds)
						break;
				}
			}
			break;
...
//				else
				...
				new_velocity.x += (drand()-16384)*2;
				new_velocity.y += (drand()-16384)*2;
				new_velocity.z += (drand()-16384)*2;
...
//				new_pos.x += (drand()-16384)*8;
//				new_pos.y += (drand()-16384)*7;
//				new_pos.z += (drand()-16384)*6;
	...
	if (obj->lifeleft <= obj->ctype.expl_info.spawn_time) {
		...
		if ((del_obj->contains_count > 0) && !(Game_mode & GM_MULTI)) { // Multiplayer handled outside of this code!!
			...
			if (robptr->contains_count) {
				if (((drand() * 16) >> 15) < robptr->contains_prob) {
					del_obj->contains_count = ((drand() * robptr->contains_count) >> 15) + 1;
...
//			mprintf(0,"Creating %d new explosions\n",new_count-old_count);
			...
			for (e=old_count;e<new_count;e++) {
				...
				vm_vec_scale_add(&pos,v1,&vv0,drand()*2);
				vm_vec_scale_add2(&pos,&vv1,drand()*2);
