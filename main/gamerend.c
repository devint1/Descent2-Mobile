...
void show_framerate()
{
	...
	gr_printf(grd_curcanv->cv_w-(7*GAME_FONT->ft_w)*f2fl(Scale_factor),grd_curcanv->cv_h-6*(GAME_FONT->ft_h+GAME_FONT->ft_h/4)*f2fl(Scale_factor),"FPS: %.2f ", f2fl(rate) );
//   if ( !( q++ % 30 ) )
//      mprintf( (0,"fps: %s\n", temp ) );
}
...
{
	//int x, y, w, h;
	grs_point cockpit_scale_pts[] = FULLSCREEN_SCALE_PTS;
	grs_point bar_scale_pts[] = {
		{ 0, i2f(max_window_h) },
		{ 0, 0 },
		{ i2f(grd_curscreen->sc_w), i2f(max_window_h) + 111 * Scale_y }
	};
	...
	switch( Cockpit_mode )	{
		...
		case CM_REAR_VIEW:
			...
			scale_bitmap(&GameBitmaps[cockpit_bitmap[Cockpit_mode+(Current_display_mode?(Num_cockpits/2):0)].index], cockpit_scale_pts, 0);
			break;
		case CM_FULL_SCREEN:
		...
		case CM_STATUS_BAR:
			gr_set_current_canvas(&VR_screen_pages[VR_current_page]);
			PIGGY_PAGE_IN(cockpit_bitmap[Cockpit_mode+(Current_display_mode?(Num_cockpits/2):0)]);
			scale_bitmap(&GameBitmaps[cockpit_bitmap[Cockpit_mode+(Current_display_mode?(Num_cockpits/2):0)].index], bar_scale_pts, 0);
			Game_window_x = (max_window_w - Game_window_w)/2;
			Game_window_y = (max_window_h - Game_window_h)/2;
			fill_background();
			break;
...
void toggle_cockpit()
{
	...
	switch (Cockpit_mode) {
		...
		case CM_FULL_COCKPIT: {
			if (Game_window_h > max_window_h)		//too big for statusbar
