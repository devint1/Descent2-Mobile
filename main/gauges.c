...
gauge_box get_gauge_box(int index) {
	...
	gauge_box gauge_boxes[] = {
		...
		{PRIMARY_W_BOX_LEFT_H,PRIMARY_W_BOX_TOP_H,PRIMARY_W_BOX_RIGHT_H,PRIMARY_W_BOX_BOT_H,N_LEFT_WINDOW_SPANS_H},
		{SECONDARY_W_BOX_LEFT_H,SECONDARY_W_BOX_TOP_H,SECONDARY_W_BOX_RIGHT_H,SECONDARY_W_BOX_BOT_H,N_RIGHT_WINDOW_SPANS_H},
		
		// sb left/right hires
		{SB_PRIMARY_W_BOX_LEFT_H,SB_PRIMARY_W_BOX_TOP_H,SB_PRIMARY_W_BOX_RIGHT_H,SB_PRIMARY_W_BOX_BOT_H,0},
		{SB_SECONDARY_W_BOX_LEFT_H,SB_SECONDARY_W_BOX_TOP_H,SB_SECONDARY_W_BOX_RIGHT_H,SB_SECONDARY_W_BOX_BOT_H,0},
	};
	...
	switch (index) {
		...
		case 4:
			...
			memcpy(gauge_boxes[4].spanlist, weapon_window_left_hires, sizeof(span) * N_LEFT_WINDOW_SPANS_H);
			break;
		case 5:
			...
			memcpy(gauge_boxes[5].spanlist, weapon_window_right_hires, sizeof(span) * N_RIGHT_WINDOW_SPANS_H);
			break;
	}
...
{	                                                                                                                                                                                                                                                             
	...
	if (last_x[2] < 0) {
		last_x[2] = SB_SCORE_RIGHT_H;
	}
	if (last_x[3] < 0) {
		last_x[3] = SB_SCORE_RIGHT_H;
	}
	...
	if (Players[Player_num].homing_object_dist >= 0) {
		if (GameTime & 0x4000) {
			...
			if (weapon_box_user[0] != WBU_WEAPON || weapon_box_user[1] != WBU_WEAPON) {
				...
				y = fmin(y,(wy - Line_spacing - Game_window_y));
			}
...
void show_bomb_count(int x,int y,int bg_color,int always_show)
{
	...
	#ifndef RELEASE
	count = fmin(count,99);	//only have room for 2 digits - cheating give 200
	#endif
...
// I hate doing this off of hard coded coords!!!!
	...
	if (Cockpit_mode == CM_STATUS_BAR) {		//draw background
		...
		if (!Current_display_mode) {
			gr_rect(169 * f2fl(Scale_x),189 * f2fl(Scale_y),189 * f2fl(Scale_x),196 * f2fl(Scale_y));
			gr_setcolor(gr_find_closest_color(10,10,10));
			gr_scanline(168 * f2fl(Scale_x),189 * f2fl(Scale_x),189 * f2fl(Scale_y));
		} else {
			gr_rect(338 * f2fl(Scale_x),453 * f2fl(Scale_y),378 * f2fl(Scale_x),470 * f2fl(Scale_y));
			gr_setcolor(gr_find_closest_color(10,10,10));
			gr_scanline(336 * f2fl(Scale_x),378 * f2fl(Scale_x),453 * f2fl(Scale_y));
		}
	}
...
void hud_show_lives()
{
	...
	if (Game_mode & GM_MULTI) {
		...
		gr_printf((10+bm->bm_w+bm->bm_w/2) * f2fl(Scale_factor), 4 * f2fl(Scale_factor), "x %d", Players[Player_num].lives-1);
	}
...
void sb_show_lives()
{
	...
	if (frc || old_lives[VR_current_page]==-1 || Players[Player_num].lives != old_lives[VR_current_page]) {
		...
		gr_rect(x, y, SB_SCORE_RIGHT, y+bm->bm_h*f2fl(Scale_factor));
		gr_rect(x, y, SB_SCORE_RIGHT, y+bm->bm_h*f2fl(Scale_factor));
		...
		if (Players[Player_num].lives-1 > 0) {
			...
			PAGE_IN_GAUGE( GAUGE_LIVES );
			scale_bitmap(bm, scale_pts, 0);
			gr_printf(x+bm->bm_w * f2fl(Scale_factor)+GAME_FONT->ft_w * f2fl(Scale_factor), y, "x %d", Players[Player_num].lives-1);
...
void draw_shield_bar(int shield)
{
	...
	grs_point scale_pts[] = {
		...
		{ 0, 0 },
		{ fl2f(SHIELD_GAUGE_X) + fixmul(i2f(bm->bm_w), Scale_x), fl2f(SHIELD_GAUGE_Y) + fixmul(i2f(bm->bm_h), Scale_y) }
	};
	scale_bitmap(bm, scale_pts, 0);
}
...
void draw_weapon_info_sub(int info_index,gauge_box *box,int pic_x,int pic_y,char *name,int text_x,int text_y)
{
	...
	if ((p=strchr(name,'\n'))!=NULL) {
		*p=0;
		...
		gr_printf(text_x,text_y+(grd_curcanv->cv_font->ft_h+1) * f2fl(Scale_y),p+1);
		*p='\n';
	} else
	{
		gr_printf(text_x,text_y,name);
...
void sb_draw_shield_num(int shield)
{
	...
	gr_rect(SB_SHIELD_NUM_X,SB_SHIELD_NUM_Y,SB_SHIELD_NUM_X+(Current_display_mode?27:13)*f2fl(Scale_x)+1,SB_SHIELD_NUM_Y+h);
	gr_string(SB_SHIELD_NUM_X + ((Current_display_mode?27:13) * f2fl(Scale_x) - w) / 2, SB_SHIELD_NUM_Y, temp);
}
...
void draw_invulnerable_ship()
{
	...
	grs_point scale_pts_cockpit[] = {
		{ fl2f(SHIELD_GAUGE_X), fl2f(SHIELD_GAUGE_Y) },
		{ 0, 0 },
		{ fl2f(SHIELD_GAUGE_X) + fixmul(i2f(70), Scale_x), fl2f(SHIELD_GAUGE_Y) + fixmul(i2f(77), Scale_y) }
	};
	grs_point scale_pts_bar[] = {
		{ fl2f(SB_SHIELD_GAUGE_X), fl2f(SB_SHIELD_GAUGE_Y) },
		{ 0, 0 },
		{ fl2f(SB_SHIELD_GAUGE_X) + fixmul(i2f(70), Scale_x), fl2f(SB_SHIELD_GAUGE_Y) + fixmul(i2f(77), Scale_y) }
	};
	...
	if (((Players[Player_num].invulnerable_time + INVULNERABLE_TIME_MAX - GameTime) > F1_0*4) || (GameTime & 0x8000)) {
		if (Cockpit_mode == CM_STATUS_BAR)	{
			...
			scale_bitmap(&GameBitmaps[GET_GAUGE_INDEX(GAUGE_INVULNERABLE+invulnerable_frame)], scale_pts_bar, 0);
		} else {
			PAGE_IN_GAUGE( GAUGE_INVULNERABLE+invulnerable_frame );
			scale_bitmap(&GameBitmaps[GET_GAUGE_INDEX(GAUGE_INVULNERABLE+invulnerable_frame)], scale_pts_cockpit, 0);
		}
...
void show_reticle(int force_big_one)
{
   ...
   if (Newdemo_state==ND_STATE_PLAYBACK && Newdemo_flying_guided)
	...
	grs_point scale_pts_cross[] = {
		{ i2f(x) + cross_offsets[ofs].x * Scale_factor, i2f(y) + cross_offsets[ofs].y * Scale_factor },
		{ 0, 0 },
		{ i2f(x) + cross_offsets[ofs].x * Scale_factor + fixmul(i2f(18), Scale_factor), i2f(y) + cross_offsets[ofs].y * Scale_factor + fixmul(i2f(17), Scale_factor) }
	};
	grs_point scale_pts_primary[] = {
		{ i2f(x) + primary_offsets[ofs].x * Scale_factor, i2f(y) + primary_offsets[ofs].y * Scale_factor },
		{ 0, 0 },
		{ i2f(x) + primary_offsets[ofs].x * Scale_factor + fixmul(i2f(62), Scale_factor), i2f(y) + primary_offsets[ofs].y * Scale_factor + fixmul(i2f(12), Scale_factor) }
	};
	grs_point scale_pts_secondary[] = {
		{ i2f(x) + secondary_offsets[ofs].x * Scale_factor, i2f(y) + secondary_offsets[ofs].y * Scale_factor },
		{ 0, 0 },
		{ i2f(x) + secondary_offsets[ofs].x * Scale_factor + fixmul(i2f(50), Scale_factor), i2f(y) + secondary_offsets[ofs].y * Scale_factor + fixmul(i2f(23), Scale_factor) }
	};

	gauge_index = (small_reticle?SML_RETICLE_CROSS:RETICLE_CROSS) + cross_bm_num;
	PAGE_IN_GAUGE( gauge_index );
	grs_bitmap *bm = &GameBitmaps[GET_GAUGE_INDEX(gauge_index)];
	...
	PAGE_IN_GAUGE( gauge_index );
	bm = &GameBitmaps[GET_GAUGE_INDEX(gauge_index)];
...
void draw_hud()
{
	...
	if (Scanline_double)		// I should be shot for this ugly hack....
		FontHires = 1;
#endif
	Line_spacing = (GAME_FONT->ft_h + GAME_FONT->ft_h/4 + 2) * f2fl(Scale_y);
#ifdef MACINTOSH
	if (Scanline_double)
		FontHires = 0;
#endif
