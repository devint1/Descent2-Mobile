...
char game_rcsid[] = "$Id: game.c 2.285 1996/12/09 14:45:35 jeremy Exp $";
...
void init_cockpit()
{
 ...
 WINDOS(
	...
	switch( Cockpit_mode )
	{
		...
		case CM_REAR_VIEW:		{
			grs_bitmap *bm = &GameBitmaps[cockpit_bitmap[Cockpit_mode + (MenuHires?(Num_cockpits/2):0)].index];
		...
		case CM_STATUS_BAR:
			...
			max_window_h = grd_curscreen->sc_h - GameBitmaps[cockpit_bitmap[CM_STATUS_BAR+(Current_display_mode?(Num_cockpits/2):0)].index].bm_h * f2fl(Scale_y);
...
void calc_frame_time()
{
	...
	{
		usleep(f2fl(f1_0 / MAX_FPS - FrameTime) / 1000000);
		timer_value = timer_get_fixed_seconds();
		FrameTime = timer_value - last_timer_value;
	}
...
void diminish_palette_towards_normal(void)
{
	...
	if (FrameTime < F1_0/DIMINISH_RATE) {
		if (drand() < FrameTime*DIMINISH_RATE/2)	//	Note: drand() is in 0..32767, and 8 Hz means decrement every frame
			dec_amount = 1;
	} else {
...
void do_ambient_sounds()
{
	...
	if (has_lava) {							//has lava
		...
		if (has_water && (drand() & 1))	//both, pick one
	...
	if (((drand() << 3) < FrameTime)) {						//play the sound
		fix volume = drand() + f1_0/2;
		...
		{ // Note the link to above!
			...
			if (Auto_fire_fusion_cannon_time) {
				if (Primary_weapon != FUSION_INDEX)
					...
					ConsoleObject->mtype.phys_info.rotvel.x += (drand() - 16384)/8;
					ConsoleObject->mtype.phys_info.rotvel.z += (drand() - 16384)/8;
		...
		if ((Players[Player_num].energy < F1_0*2) && (Auto_fire_fusion_cannon_time == 0)) {
			...
			if (Fusion_next_sound_time < GameTime) {
				if (Fusion_charge > F1_0*2) {
					...
					apply_damage_to_player(ConsoleObject, ConsoleObject, drand() * 4);
				} else {
				...
				Fusion_next_sound_time = GameTime + F1_0/8 + drand()/4;
			}
		}
	}
