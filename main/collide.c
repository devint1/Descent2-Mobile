...
void collide_player_and_wall( object * playerobj, fix hitspeed, short hitseg, short hitwall, vms_vector * hitpt)
{
	...
	if (TmapInfo[tmap_num].flags & TMI_FORCE_FIELD) {
		...
		//knock player around
		force.x = 40*(drand() - 16384);
		force.y = 40*(drand() - 16384);
		force.z = 40*(drand() - 16384);
...
int check_volatile_wall(object *obj,int segnum,int sidenum,vms_vector *hitpt)
{
	...
	if (d > 0 || water) {
		...
		if (obj->id == Player_num) {
			...
			obj->mtype.phys_info.rotvel.x = (drand() - 16384)/2;
			obj->mtype.phys_info.rotvel.z = (drand() - 16384)/2;
		}
	...
	if (Boss_invulnerable_spot[d2_boss_index]) {
		...
		if (dot > Boss_invulnerable_dot) {
			...
			if (Last_time_buddy_gave_hint == 0)
				Last_time_buddy_gave_hint = drand()*32 + F1_0*16;
	...
	if ( ((weapon->ctype.laser_info.parent_type==OBJ_PLAYER) || Robots_kill_robots_cheat) && !(robot->flags & OF_EXPLODING) )	{	
		...
		if ((robot != NULL) && (!Robot_info[robot->id].companion) && (!Robot_info[robot->id].boss_flag) && (weapon->id == GAUSS_ID)) {
			...
			if (aip->SKIP_AI_COUNT * FrameTime < F1_0) {
				aip->SKIP_AI_COUNT++;
				robot->mtype.phys_info.rotthrust.x = fixmul((drand() - 16384), FrameTime * aip->SKIP_AI_COUNT);
				robot->mtype.phys_info.rotthrust.y = fixmul((drand() - 16384), FrameTime * aip->SKIP_AI_COUNT);
				robot->mtype.phys_info.rotthrust.z = fixmul((drand() - 16384), FrameTime * aip->SKIP_AI_COUNT);
...
//	mprintf((0, "In drop_player_eggs...\n"));
	...
	if ((playerobj->type == OBJ_PLAYER) || (playerobj->type == OBJ_GHOST)) {
		...
		rthresh = 30000;
		while ((Players[playerobj->id].secondary_ammo[SMART_MINE_INDEX]%4==1) && (drand() < rthresh)) {
			int			newseg;
			vms_vector	tvec;
		...
		{
			rthresh = 30000;
			while ((Players[playerobj->id].secondary_ammo[PROXIMITY_INDEX]%4==1) && (drand() < rthresh)) {
				int			newseg;
				vms_vector	tvec;
