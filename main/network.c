...
static char rcsid[] = "$Id: network.c 2.209 1996/12/19 11:26:34 jason Exp $";
...
//      mprintf ((0,"Refuse = %d\n",game->RefusePlayers));
	...
	for (i = 0; i < num_players; i++) {
		if (Network_game_type == IPX_GAME) {
			if ( (!strcasecmp(Players[Player_num].callsign, people->players[i].callsign)) &&
...
void network_welcome_player(sequence_packet *their)
{
   ...
   // If hoard game, and this guy isn't D2 Christmas (v1.2), dump him
	...
	for (i = 0; i < N_players; i++)
	{
		if ( (Network_game_type == IPX_GAME) && (!strcasecmp(Players[i].callsign, their->player.callsign )) && (!memcmp(Players[i].net_address,local_address, 6)) )
		{
			player_num = i;
			break;
		}
...
void network_process_dump(sequence_packet *their)
{
   ...
   if (their->player.connected!=7)
		...
		{
		 for (i=0;i<N_players;i++)
			if (!strcasecmp(their->player.callsign,Players[i].callsign))
			{
...
void network_process_request(sequence_packet *their)
{
	...
	for (i = 0; i < N_players; i++) {
		if (Network_game_type == IPX_GAME) {
			if (!memcmp(their->player.network.ipx.server, NetPlayers.players[i].network.ipx.server, 4) && !memcmp(their->player.network.ipx.node, NetPlayers.players[i].network.ipx.node, 6) && (!strcasecmp(their->player.callsign, NetPlayers.players[i].callsign))) {
				Players[i].connected = 1;
				break;
			}
		} else {
			if ( (their->player.network.appletalk.node == NetPlayers.players[i].network.appletalk.node) && (their->player.network.appletalk.net == NetPlayers.players[i].network.appletalk.net) && (!strcasecmp(their->player.callsign, NetPlayers.players[i].callsign)) ) {
				Players[i].connected = 1;
				break;
			}
		}
  ...
  Netgame.RefusePlayers=m[opt_refuse].value;
   ...
   Netgame.max_numplayers=MaxNumNetPlayers;
		...
		for (j = 0; j < num_active_games; j++)
			if (!strcasecmp(Active_games[j].game_name, name))
			{
...
//              Players[i].net_killed_total = 0;
	...
	for (i=0; i<N_players; i++ )    {
		if (Network_game_type == IPX_GAME) {
			if ( (!memcmp( TempPlayersInfo->players[i].network.ipx.node, My_Seq.player.network.ipx.node, 6 )) && (!strcasecmp( TempPlayersInfo->players[i].callsign, temp_callsign)) ) {
				if (Player_num!=-1) {
			...
			if ( (TempPlayersInfo->players[i].network.appletalk.node == My_Seq.player.network.appletalk.node) && (!strcasecmp( TempPlayersInfo->players[i].callsign, temp_callsign)) ) {
				if (Player_num!=-1) {
					Int3(); // Hey, we've found ourselves twice
					Network_status = NETSTAT_MENU;
					return; 
...
network_send_sync(void)
{
	...
	for (i=0; i<NumNetPlayerPositions; i++ )        
	{
		...
		if (Game_mode & GM_MULTI_COOP)
			...
			do 
			{
				np = drand() % NumNetPlayerPositions;
				for (j=0; j<i; j++ )    
				{
...
// LoadLevel(level); Old, no longer used.
   ...
   srand( timer_get_fixed_seconds() );
   Netgame.Security=drand();  // For syncing Netgames with player packets
