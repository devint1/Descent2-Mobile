...
void do_muzzle_stuff(int segnum, vms_vector *pos)
{
	...
	memcpy(&Muzzle_data[Muzzle_queue_index].pos, pos, sizeof(vms_vector));
	Muzzle_queue_index++;
...
int Laser_create_new( vms_vector * direction, vms_vector * position, int segnum, int parent, int weapon_type, int make_sound )
{
	...
	if (Weapon_info[obj->id].speedvar != 128) {
		...
		randval = F1_0 - ((drand() * Weapon_info[obj->id].speedvar) >> 6);
	...
	if ((obj->type == OBJ_WEAPON) && (obj->id == FLARE_ID))
		obj->lifeleft += (drand()-16384) << 2;		//	add in -2..2 seconds
...
int do_laser_firing(int objnum, int weapon_num, int level, int flags, int nfires)
{
	...
	switch (weapon_num) {
		...
		case VULCAN_INDEX: {
			...
			//if (drand() > 24576)
			//	make_sound = 1;
			Laser_player_fire_spread( objp, VULCAN_ID, 6, drand()/8 - 32767/16, drand()/8 - 32767/16, make_sound, 0);
			if (nfires > 1) {
				Laser_player_fire_spread( objp, VULCAN_ID, 6, drand()/8 - 32767/16, drand()/8 - 32767/16, 0, 0);
				if (nfires > 2) {
					Laser_player_fire_spread( objp, VULCAN_ID, 6, drand()/8 - 32767/16, drand()/8 - 32767/16, 0, 0);
				}
			}
...
//			mprintf((0, "Fusion multiplier %f.\n", f2fl(Fusion_charge)));
			...
			force_vec.x = (force_vec.x >> 4) + drand() - 16384;
			force_vec.y = (force_vec.y >> 4) + drand() - 16384;
			force_vec.z = (force_vec.z >> 4) + drand() - 16384;
		...
		case GAUSS_INDEX: {
			...
			Laser_player_fire_spread( objp, GAUSS_ID, 6, (drand()/8 - 32767/16)/5, (drand()/8 - 32767/16)/5, make_sound, 0);
			if (nfires > 1) {
				Laser_player_fire_spread( objp, GAUSS_ID, 6, (drand()/8 - 32767/16)/5, (drand()/8 - 32767/16)/5, 0, 0);
				if (nfires > 2) {
					Laser_player_fire_spread( objp, GAUSS_ID, 6, (drand()/8 - 32767/16)/5, (drand()/8 - 32767/16)/5, 0, 0);
				}
			}
...
// -- 		}
		...
		for (i=0; i<num_smart_children; i++) {
			int objnum;
			objnum = (numobjs==0)?-1:objlist[(drand() * numobjs) >> 15];
