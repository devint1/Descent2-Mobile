...
void render_endlevel_frame(fix f) {Int3();}
...
do_endlevel_frame()
{
	...
	if (!outside_mine) {
		...
		if ((explosion_wait1-=FrameTime) < 0) {
			...
			vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.rvec,(drand()-D_RAND_MAX/2)*15);
			vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.uvec,(drand()-D_RAND_MAX/2)*15);
			...
			if (segnum != -1) {
				...
				if (drand()<10000 || ++sound_count==7) {		//pseudo-random
					digi_link_sound_to_pos( SOUND_TUNNEL_EXPLOSION, segnum, 0, &tpnt, 0, F1_0 );
					sound_count=0;
				}
			}

			explosion_wait1 = 0x2000 + drand()/4;
	...
	if (Endlevel_sequence >= EL_FLYTHROUGH && Endlevel_sequence < EL_OUTSIDE)
		if ((explosion_wait2-=FrameTime) < 0) {
			...
			vm_vec_copy_scale(&tpnt,&ConsoleObject->orient.rvec,(drand()-D_RAND_MAX/2)*100);
			vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.uvec,(drand()-D_RAND_MAX/2)*100);
			vm_vec_add2(&tpnt,&ConsoleObject->pos);

			if (Endlevel_sequence == EL_FLYTHROUGH)
				vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.fvec,drand()*200);
			else
				vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.fvec,drand()*60);
			...
			if (hit_data.hit_type==HIT_WALL && hit_data.hit_seg!=-1)
				object_create_explosion(hit_data.hit_seg,&hit_data.hit_pnt,i2f(3)+drand()*6,VCLIP_SMALL_EXPLOSION);

			explosion_wait2 = (0xa00 + drand()/8)/2;
		}
...
generate_starfield()
{
	...
	for (i=0;i<MAX_STARS;i++) {
		...
		stars[i].x = (drand() - D_RAND_MAX/2) << 14;
		stars[i].z = (drand() - D_RAND_MAX/2) << 14;
		stars[i].y = (drand()/2) << 14;
