...
int descent_critical_error_handler( unsigned deverr, unsigned errcode, unsigned* devhdr );
...
int descent_critical_error_handler(unsigned deverror, unsigned errcode, unsigned* devhdr )
{
	devhdr = devhdr;
...
void print_commandline_help()
{
	...
	while (cfgets(line,LINE_LEN,ifile)) {
		...
		if (isatty(stdout) && ++line_count == screen_lines-1) {
			char c;
...
void do_headset_init()
{
	...
	case VRD_VFX1: {
		...
		if (!status) {
			...
			game_init_render_buffers(0, 320, 200, VR_INTERLACED, VRF_USE_PAGING );
			//VR_low_res = 3;
	...
	case VRD_CYBERMAXX:
		...
		Game_victor_flag = 1;
		game_init_render_buffers(0, 320, 100, VR_INTERLACED, VRF_COMPATIBLE_MENUS );
		VR_low_res = 0;
		break;
	case VRD_CYBERMAXX2:
		if ( Config_vr_tracking>0 )
	...
	case VRD_3DMAX:
		if ( Config_vr_resolution==1 )	{
			...
			game_init_render_buffers(0, 320, 200, VR_INTERLACED, 0);
			VR_low_res = 0;
		} else {
			Game_3dmax_flag = 1;
			game_init_render_buffers(0, 320, 100, VR_INTERLACED, 0 );
			VR_low_res = 0;
		}
		break;
	case VRD_LCDBIOS:
	...
	case VRD_SIMULEYES:
		...
		case 0:	game_init_render_buffers(0, 320, 100, VR_INTERLACED, 0 );	break;
		case 1:	game_init_render_buffers(0, 320, 200, VR_INTERLACED, VRF_USE_PAGING ); break;
		case 2:	game_init_render_buffers(0, 640, 200, VR_INTERLACED, 0 ); break;
		case 3:	game_init_render_buffers(0, 640, 240, VR_INTERLACED, 0 ); break;
		case 4:	game_init_render_buffers(0, 800, 300, VR_INTERLACED, 0 ); break;
		}
		VR_low_res = 0;
		break;
	}
...
int descent_main(int argc,char **argv)
{
...
//    Current_display_mode = -1;
	...
	if ((i = FindArg("-width"))) {
		screen_width = atoi(Args[i + 1]);
	}
	if ((i = FindArg("-height"))) {
		screen_height = atoi(Args[i + 1]);
	}
	
    game_init_render_buffers(0, screen_width, screen_height, VR_NONE, VRF_COMPATIBLE_MENUS+VRF_ALLOW_COCKPIT );
//#else
//	do_headset_init();
...
//#endif
	...
	if ((t=gr_init(screen_width, screen_height))!=0)				//doesn't do much
...
#if !defined(POLY_ACC)
	if (FindArg("-nohires") || FindArg("-nohighres") || disable_high_res)
   ...
   PA_DFX (pa_splash());
	...
	mprintf( (0, "\nShowing loading screen..." ));
	{
		...
		if ((pcx_error=pcx_read_bitmap( filename, &title_bm, BM_LINEAR, title_pal ))==PCX_ERROR_NONE)	{
			vfx_set_palette_sub( title_pal );
			scale_bitmap(&title_bm, scale_pts, 0);
			gr_palette_fade_in( title_pal, 32, 0 );
