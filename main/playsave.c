...
int read_player_file()
{
	...
	sprintf(filename,"%s/%s.plr",Document_path,Players[Player_num].callsign);
	...
	if (file && isatty(fileno(file))) {
		...
		fclose(file);
		sprintf(filename,"%s/$%.7s.plr",Document_path,Players[Player_num].callsign);
		file = fopen(filename,"rb");
	}
...
{
	char filename[FILENAME_MAX];		// because of ":Players:" path
	FILE *file;
	int errno_ret,i;
