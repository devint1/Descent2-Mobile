...
static char rcsid[] = "$Id: newmenu.c 2.133 1997/01/24 17:48:48 jeremy Exp $";
...
#define MAX_TEXT_WIDTH 	(200 * f2fl(Scale_factor))				// How many pixels wide a input box can be
...
void nm_remap_background()
{
	if (!Newmenu_first_time) {
		if (!nm_background.bm_data)
			nm_background.bm_data = malloc(nm_background.bm_w * nm_background.bm_h);
...
void nm_draw_background1(char * filename)
{
	...
	gr_copy_palette(gr_current_pal, pal, sizeof(gr_current_pal));
...
void nm_draw_background(int x1, int y1, int x2, int y2 )
{
	...
	grs_point scale_vec[] = {
		{ i2f(x1), i2f(y1) },
		{ i2f(nm_background.bm_w), i2f(nm_background.bm_h) },
		{ i2f(x2), i2f(y2)}
	};

	scale_bitmap(&nm_background, scale_vec, 0);
	...
	if (!No_darkening)
	{
		...
		for (int i = ceil((double)x2 / (double)nm_background.bm_w) * 5; i >= 0; --i) {
			gr_urect(x2 - i, y1 + i, x2 - i, y2 - i);
		}
		
		for (int i = ceil((double)y2 / (double)nm_background.bm_h) * 5; i >= 0; --i) {
			gr_urect(x1 + i, y2 - i, x2, y2 - i);
		}
	}
...
void nm_string_slider( bkg * b, int w1,int x, int y, char * s )
{
	...
	gr_get_string_size(s, &w, &h, &aw  );
	// CHANGED
	gr_bm_bitblt(b->background->bm_w-(f2fl(Scale_x) * 15), h, 5 * f2fl(Scale_factor), y, 5 * f2fl(Scale_factor), y, b->background, &(grd_curcanv->cv_bitmap) );
		...
		if (p)	{
			...
			// CHANGED
			gr_bm_bitblt(w, f2i(Scale_factor), x+w1-w, y, x+w1-w, y, b->background, &(grd_curcanv->cv_bitmap) );
			// CHANGED
			gr_bm_bitblt(w, f2i(Scale_factor)+1, x+w1-w, y+h-f2i(Scale_factor), x+w1-w, y, b->background, &(grd_curcanv->cv_bitmap) );
...
WIN(DDGRLOCK(dd_grd_curcanv));
	if (time & 0x8000)
		...
		gr_setcolor( BM_XRGB(0,0,0) );
		gr_rect( x, y, x+grd_curcanv->cv_font->ft_w*f2fl(Scale_factor)-1, y+grd_curcanv->cv_font->ft_h*f2fl(Scale_factor)-1 );
	}
WIN(DDGRUNLOCK(dd_grd_curcanv));
}
...
int get_item_at_menu_pos(int x, int y, int nitems, newmenu_item *item) {
	for(int i = 0; i < nitems; ++i) {
		if(item[i].x <= x && item[i].x + item[i].w >= x &&
		   item[i].y <= y && item[i].y + item[i].h >= y) {
			return i;
		}
	}
	return -1;
...
int check_button_press()
{
	...
	case	CONTROL_CYBERMAN:
		...
		for (i=0; i<3; i++ )	
			if (mouse_button_down_count(i, &x, &y)>0) return 1;
		break;
	#endif
...
RePaintNewmenu4:
	...
	for (i=0; i<nitems; i++ )	{
		...
		if (SurfingNet)
			string_height+=LHY(3)*f2fl(Scale_factor);
		...
		if ( item[i].type == NM_TYPE_INPUT )	{
			...
			string_width *= f2fl(Scale_factor);
	...
	if ( width > -1 )
		w = width * f2fl(Scale_factor);
	
	if ( height > -1 )
		h = height * f2fl(Scale_factor);
	...
	if (right_offset > 0 )
		right_offset += 3 * f2fl(Scale_factor);
	...
	w += (MenuHires?60:30) * f2fl(Scale_factor);
	h += (MenuHires?60:30) * f2fl(Scale_factor);
	...
	// Shift the menu up to leave room for the iOS keyboard
	if(shifted_up) {
		y -= h;
	}
	...
	for (i=0; i<nitems; i++ )	{
		item[i].x = (MenuHires?30:15) * f2fl(Scale_factor) + twidth + right_offset;
		item[i].y += (MenuHires?30:15) * f2fl(Scale_factor);
		if ( item[i].type==NM_TYPE_RADIO )	{
...
typedef struct filename_item {
	int x, y, w, h;
} filename_item;

int get_filename_item_at_pos(int x, int y, int nitems, filename_item *items) {
	for(int i = 0; i < nitems; ++i) {
		if(x > items[i].x && y > items[i].y && x < items[i].x + items[i].w && y < items[i].y + items[i].h) {
			return i;
		}
	}
	return -1;
	...
	if ((dir = opendir( Document_path )) != NULL)	{
		while ((ent = readdir( dir )) != NULL) {
			if (NumFiles<MAX_FILES)	{
				if(!strcasestr(ent->d_name, &filespec[1])) continue;
				strncpy( &filenames[NumFiles*14], ent->d_name, FILENAME_LEN );
				if ( player_mode )	{
					char * p;
...
RePaintNewmenuFile:
		...
		if ( title ) {
			...
			if ( w > w_w )
				w_w = w;
			title_height = h + (grd_curfont->ft_h*2) * f2fl(Scale_factor);		// add a little space at the bottom of the title
		}

		box_w = w_w;
		box_h = ((grd_curfont->ft_h + 2) * f2fl(Scale_factor) * NumFiles_displayed);

		w_w += (grd_curfont->ft_w * 4) * f2fl(Scale_factor);
		w_h = title_height + box_h + (grd_curfont->ft_h * 2) * f2fl(Scale_factor);		// more space at bottom
		...
		box_x = w_x + (grd_curfont->ft_w*2) * f2fl(Scale_factor);			// must be in sync with w_w!!!
...
// save the screen behind the menu.
		...
		WIN(DDGRLOCK(dd_grd_curcanv))
		{	
			gr_string( 0x8000, w_y+10*f2fl(Scale_factor), title );
		}
		WIN(DDGRUNLOCK(dd_grd_curcanv));
	...
	WIN(DDGRLOCK(dd_grd_curcanv));
		...
		if (ofirst_item != first_item)	{
			...
			for (i=first_item; i<first_item+NumFiles_displayed; i++ )	{
				int w, h, aw, y;
				y = (i-first_item)*((grd_curfont->ft_h + 1) * f2fl(Scale_factor)) + box_y;
				if ( i >= NumFiles )	{

					gr_setcolor( BM_XRGB(5,5,5));
					gr_rect( box_x + box_w, y-1, box_x + box_w, y + (grd_curfont->ft_h + 1) * f2fl(Scale_factor));
					//gr_rect( box_x, y + grd_curfont->ft_h + 2, box_x + box_w, y + grd_curfont->ft_h + 2);
					
					gr_setcolor( BM_XRGB(2,2,2));
					gr_rect( box_x - 1, y - 1, box_x - 1, y + (grd_curfont->ft_h + 2) * f2fl(Scale_factor) );
					
					gr_setcolor( BM_XRGB(0,0,0));
					gr_rect( box_x, y - 1, box_x + box_w - 1, y + (grd_curfont->ft_h + 1) * f2fl(Scale_factor));
				  ...
				  //	gr_rect( box_x, y + h + 2, box_x + box_w, y + h + 2);
					gr_rect( box_x + box_w, y - 1, box_x + box_w, y + h + f2fl(Scale_factor));
					
					gr_setcolor( BM_XRGB(2,2,2));
					gr_rect( box_x - 1, y - 1, box_x - 1, y + h + f2fl(Scale_factor));
					gr_setcolor( BM_XRGB(0,0,0));
							
					gr_rect( box_x, y-1, box_x + box_w - 1, y + h + f2fl(Scale_factor) );
...
int newmenu_filelist( char * title, char * filespec, char * filename )
{
	...
	if ((dir = opendir( Document_path )) != NULL)	{
		while ((ent = readdir( dir )) != NULL) {
			if (NumFiles<MAX_FILES)	{
				if(!strcasestr(ent->d_name, &filespec[1])) continue;
				strncpy( FilenameText[NumFiles], ent->d_name, FILENAME_LEN);
