...
void init_interface_vars_to_assembler(void)
{
	...
	write_buffer = (size_t) bp->bm_data;
...
void ntexture_map_lighted(grs_bitmap *srcb, g3ds_tmap *t)
{
	...
	// Without this it will crash at fuel centers (and maybe other spots?)
	xleft = xright = uleft = vleft = uright = vright = zleft = zright = lleft = lright = 0;
