...
#ifdef ANDROID_NDK
extern SLEngineItf engine;
extern SLDataSink dst;
...
static int create_audiobuf_handler(unsigned char major, unsigned char minor, unsigned char *data, int len, void *context)
{
	...
	format.numChannels = stereo ? 2 : 1;
	format.samplesPerSec = (SLuint32) mve_audio_samplerate * 1000;
	format.bitsPerSample = bitsize == 1 ? SL_PCMSAMPLEFORMAT_FIXED_16 : SL_PCMSAMPLEFORMAT_FIXED_8;
	format.containerSize = format.bitsPerSample;
	format.channelMask = SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT;
	format.endianness = SL_BYTEORDER_LITTLEENDIAN;
	in_loc.locatorType = SL_DATALOCATOR_ANDROIDSIMPLEBUFFERQUEUE;
	in_loc.numBuffers = TOTAL_AUDIO_BUFFERS;
	src.pLocator = &in_loc;
	src.pFormat = &format;
	...
	result = (*player_obj)->GetInterface(player_obj, SL_IID_ANDROIDSIMPLEBUFFERQUEUE, &buffer_queue);
	if (result != SL_RESULT_SUCCESS) {
		mve_audio_canplay = 0;
		return 0;
	}
...
#ifdef ANDROID_NDK
			if (mve_audio_buffers[mve_audio_bufhead]) {
				mve_free(mve_audio_buffers[mve_audio_bufhead]);
			}
			mve_audio_buffers[mve_audio_bufhead++] = temp_data;
			if (mve_audio_bufhead == TOTAL_AUDIO_BUFFERS) {
				mve_audio_bufhead = 0;
			}
			(*buffer_queue)->Enqueue(buffer_queue, temp_data, nsamp);
#else
